...
### Multi-GPU NCCL Timeouts and Memory Constraints

When loading large datasets across multiple GPUs, avoid `keep_in_memory=True`. It causes worker-level memory duplication. If relying on memory-mapping (default) causes NCCL timeouts due to slow disk I/O, you must either: A) Move cache storage to high-throughput NVMe SSDs, B) Shard the dataset precisely per GPU before instantiation, or C) Switch to `streaming=True`.
